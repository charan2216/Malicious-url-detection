...
if __name__ == '__main__':
    ...
    clf = joblib.load('E:/Gopal/malware detection/Classifier/classifier.pkl')
    features = pickle.loads(open(os.path.join('E:/Gopal/malware detection/Classifier/features.pkl'), 'rb').read())

    # Extracting features from the PE file mentioned in the argument
    pe_file_path = sys.argv[1]
    print("Processing PE file:", pe_file_path)
    
    data = extract_infos(pe_file_path)
    if not data:
        print("Failed to extract information from the PE file.")
        sys.exit(1)
    ...
    print('The file %s is %s' % (os.path.basename(pe_file_path), ['malicious', 'legitimate'][res]))
